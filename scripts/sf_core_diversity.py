...
def calculate_diversity( files_list, file_path, species, scratch=''):
    """ calculate_diversity
    """
    diversity_dict=defaultdict()
    for input_filepath in files_list:
        try:
            myTree = mpm_tree(input_filepath, speciesID=species, scratch=scratch)
            myTree.codon_align()
...
def estimate_core_gene_diversity(path, folders_dict, strain_list, parallel,
                                 core_cutoff, factor_core_diversity, species, scratch=''):
    """
    ...
    multips(calculate_diversity, parallel, tmp_fa_files, tmp_core_seq_path, species, scratch=scratch)
