...
def quick_align_makeTree(input_cluster_filepath, scratch=''):
    """
    ...
    try:
        myTree = mpm_tree(input_cluster_filepath, scratch=scratch)
        myTree.codon_align()
        myTree.translate()
...
def cutTree_outputCluster( file_list, file_path, cut_branch_threshold, treefile_used, scratch=''):
    """
    ...
    for input_filepath in file_list:
        ...
        if treefile_used==True:
            ...
            tree= quick_align_makeTree(input_filepath, scratch=scratch)
...
def postprocess_split_long_branch(parallel, path, simple_tree, cut_branch_threshold=0.3, scratch=''):
    """
    ...
    treefile_used=True
    multips(cutTree_outputCluster, parallel, tree_fname_list, file_path, cut_branch_threshold, treefile_used, scratch=scratch)
