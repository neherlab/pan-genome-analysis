...
def resolve_polytomies(infileName, outfileName):
    newickString=open(infileName, 'r').readline().rstrip().replace('[&R] ', '')
...
def aln_to_Newick(path, folders_dict, raxml_timelimit, raxml_path, threads):
    ...
    cmd = fasttree_program+' -gtr -nt -gamma -nosupport -mlacc 2 -slownni '+SNP_matrix_path+' > initial_tree.newick0 2> '+log_path+'fasttree.log'
    print(' tree building command:', cmd)
    os.system(cmd)
