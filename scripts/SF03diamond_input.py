...
def gbk_translation(each_gb_path, gb_file, output_filename, locusTag_to_geneId_Dt):
    gb= each_gb_path + gb_file
    strainName=gb_file.split('.')[0]
    geneFasta= each_gb_path + strainName +'.fastaCpk'; geneFastaDict=defaultdict()
    filename_write=open(output_filename, 'wb')
    flag=0; contig_index=0
    for genome in SeqIO.parse(gb,'genbank'):
        contig_index+=1
        for feature in genome.features:  
            ...
            if feature.type=='gene' and 'order' not in loc and 'pseudo' not in feature.qualifiers:
                ...
                flag=0;
    write_pickle(geneFasta, geneFastaDict)
